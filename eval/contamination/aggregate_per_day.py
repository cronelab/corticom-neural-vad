...
from corticom.auxilary.vad import EnergyBasedVad
from corticom.auxilary.preprocessing import SelectElectrodesFromBothGrids
...
def aggregate_data(speech_corpus_root: Path, agg_path: Path, timing_path: Path):
    ...
    """
    feature_files = list(speech_corpus_root.rglob('50word_Overt_R*.mat'))
    ...
    for day in groups_by_day.keys():
        ...
        fname = os.path.join(agg_path.as_posix(), f"{day}_50word_Overt.mat")
        savemat(fname, mdict, format='5', long_field_names=False)

        mdict = dict(timings=timings)
        fname = os.path.join(timing_path.as_posix(), f"{day}_50word_Overt_timings.mat")
        savemat(fname, mdict, format='5', long_field_names=False)
